<div align="center">
  ...
  **酷态科 AP01 万向屏的跨平台工具链，以及原生 macOS 控制器。**
  ...
  [![Native App](https://img.shields.io/badge/Native_App-macOS_only-0F172A?logo=apple)](#方法一安装-macos-软件)
  ...
  [平台支持](#平台支持) · [Windows 指南](docs/WINDOWS_GUIDE.zh-CN.md) · [安装前准备](docs/PREPARATION_CHECKLIST.zh-CN.md) · [零基础教程](docs/BEGINNER_GUIDE.zh-CN.md) · [安装 macOS 软件](#方法一安装-macos-软件) · [交给 Coding Agent 配置](#方法二把-github-仓库交给-coding-agent)
...
| | 方法一：安装 macOS 软件 | 方法二：在 macOS / Windows 交给 Coding Agent |
| --- | --- | --- |
...
| 额度面板 | Claude 与 Codex 实时额度 | 可修改 UI；自动读取官方账号目前为 macOS 集成 |
...
- **原生 CUKTECH Screen Controller 图形软件**目前只有 macOS Apple Silicon
  安装包；
- **Python 脚本与 Coding Agent 工具链**同时支持 macOS 和 Windows，可完成
  图片/GIF 转换、局域网 Bridge、`/health` 检查和已改造 AP01 的日常刷新；
- Windows 使用 `scripts/setup-windows.ps1` 和
  `scripts/diagnose-windows.ps1`，不运行 `macos/` 目录中的脚本；
- 当前内置的 Claude Desktop Cookie/Keychain 自动额度读取仍面向 macOS。
  Windows 可以显示自定义内容、手工数据或其他程序/API 生成的画面。
...
然后安装并启动 Bridge。macOS 可配置 Claude/Codex 自动额度面板；Windows
先把我的图片或其他数据转换为 320×240 的 AP01 兼容 GIF。验证 /health 和
...
当前自动读取官方 Claude Desktop/Codex 登录态的实现依赖 macOS；Windows 的
图片转换和 Bridge 不受影响。
...
- Claude 与 Codex 数据来自本机已经登录的官方客户端账户；当前自动读取支持 macOS。
- Session 凭据仅保留在内存中。
- 输出 JSON 只包含额度数据。
...
scripts/*windows.ps1       Windows 环境配置与只读诊断
```

## 开发

```bash
.venv/bin/python -m unittest -v test_quota_dashboard.py test_ap01_install_firmware.py test_platform_support.py
