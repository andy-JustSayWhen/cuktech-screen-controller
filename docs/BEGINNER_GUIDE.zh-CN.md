...
- Mac 和 AP01 连接同一个可互相访问的局域网，不能使用访客网络；
- 需要显示额度时，提前安装并登录 Claude Desktop 和官方 Codex/ChatGPT App；
- 首次安装软件时保持网络连接；
- 如果 macOS 询问是否允许接收网络连接，请点击“允许”；
...
- **已经改造，显示自己的图片：** 只要 Mac 与 AP01 的局域网正常，不需要外网；
- **显示 Claude/Codex 额度：** Mac 需要联网获取最新额度，AP01 只需能访问 Mac；
- **完全原厂屏首次安装加载器：** AP01 必须在米家在线，Mac 和 AP01 都要联网；
