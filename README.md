<div align="center">
  ...
  **A cross-platform AP01 toolkit with a native macOS controller.**
  ...
  [![Native App](https://img.shields.io/badge/Native_App-macOS_only-0F172A?logo=apple)](#method-1--install-the-macos-app)
  ...
  [Platform support](#platform-support) · [Windows guide](docs/WINDOWS_GUIDE.md) · [Preparation checklist](docs/PREPARATION_CHECKLIST.md) · [Beginner guide](docs/BEGINNER_GUIDE.md) · [Install the macOS app](#method-1--install-the-macos-app) · [Use with a coding agent](#method-2--give-this-repository-to-a-coding-agent)
...
| | Method 1: macOS app | Method 2: coding agent on macOS / Windows |
| --- | --- | --- |
...
| Quota dashboard | Claude and Codex live usage | Configurable renderer; automatic account reading currently integrates with macOS |
...
The project itself is not macOS-only:

- the native **CUKTECH Screen Controller GUI** is currently packaged for
  Apple Silicon macOS only;
- the **Python and coding-agent toolkit** supports macOS and Windows for image
  conversion, GIF validation, the LAN Bridge, health checks, and normal refresh
  of an already-patched AP01;
- Windows uses `scripts/setup-windows.ps1` and
  `scripts/diagnose-windows.ps1` instead of the scripts under `macos/`;
- automatic Claude Desktop cookie/Keychain quota collection is currently a
  macOS integration. Windows can serve custom artwork, manual data, or screens
  generated by another local API/program.
...
Then install the Bridge. On macOS, configure the automatic Claude/Codex quota
dashboard; on Windows, convert my image or another data source into an AP01
screen. Verify /health and an AP01 GET /screen.gif 200 request, and
...
CUKTECH Screen Controller is a native macOS app plus a cross-platform toolkit for the detachable display used
...
Automatic account discovery for the official Claude Desktop/Codex apps is
currently the macOS-specific part; Windows image conversion and Bridge serving
...
- Claude and Codex data is fetched from accounts signed in on the local Mac;
  the current automatic account integration is macOS-specific.
...
scripts/*windows.ps1       Windows setup and read-only diagnostics
```
